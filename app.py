...
with col5: df_c5 = df_filtered.copy() col_status, col_amt ='Status Reimburse Actual', 'NET AMOUNT' if col_status in df_c5.columns and col_amt in df_c5.columns: status_clean =df_c5[col_status].astype(str).str.upper().str.strip() mask_paid = status_clean == 'PAID' val_payin_huawei = df_c5[mask_paid][col_amt].sum() mask_ny = status_clean.isin(['DN ISSUED', 'NY ISSUE DN']) ny_val = df_c5[mask_ny][col_amt].sum() create_compact_donut_card("Total Pay In To Huawei", val_payin_huawei, ny_val, key="kpi_5")
st.markdown("---")
